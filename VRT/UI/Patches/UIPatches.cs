...
namespace VRMaker
{
    ...
    class UIPatches
    {
        ...
        [HarmonyPrefix]
        [HarmonyPatch(typeof(Kingmaker.UI.Canvases.MainCanvas), "OnEnable")]
        private static bool MoveCanvasesToWorldSpace(Kingmaker.UI.Canvases.MainCanvas __instance)
        {
            try
            {
                ...
                if (!VRT.Main.MyHelper)
                    VRT.Main.MyHelper = MBHelper.Create();
                ...
                //else
                StaticUiTarget MyUiTarget = StaticUiTarget.Create(VRT.Main.MyHelper.gameObject.transform);
